...
// 出力ファイル（テスト用）
const OUTPUT_FILE = path.resolve(__dirname, 'frozen_users_test.json');
...
async function main() {
  ...
  // テスト対象まとめIDをここに指定
  const MATOME_ID_TEST = '2639595'; // ←ここを特定のまとめIDに変更

  const users = await fetchCommentUsers(MATOME_ID_TEST);
